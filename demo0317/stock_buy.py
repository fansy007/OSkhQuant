...
配置文件: demo0317/buy_config.txt
运行方式: python demo0317/stock_buy.py
"""
import sys
import os
import importlib
...
CSV_PATH = os.path.join(PROJECT_ROOT, 'demo0317', 'financial_data', 'financial_data.csv')
CONFIG_PATH = os.path.join(PROJECT_ROOT, 'demo0317', 'buy_config.txt')
...
def load_config():
    ...
    with open(CONFIG_PATH, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            elif cond_type == 'min_buy_amount':
                config['min_buy_amount'] = int(parts[1])
    ...
    Returns:
        {
            ...
            'config': config                # 配置信息（包含min_buy_amount）
        }
    """
    global _xtdata
...
def main():
    ...
    print(f"买入门槛: {result['config']['min_buy_amount']}")
